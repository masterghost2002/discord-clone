...
export default function ChatItem({
    ...
    // use effect to add click event so, that when ever use click outside of the
    // message input form it will setIsEditable(false)
    // this will provide better user experiance for mobile user
    const messageRef = useRef<HTMLDivElement>(null);
    useEffect(() => {
        const isClickOutSide = (e: any) => {
            if (messageRef && messageRef.current?.contains(e.target)) return;
            setIsEditing(false);
            return true;
        }
        ...
        >
            ...
            >
                ...
                >
                    ...
                    {/* if we have a file url and the type is image */}
                    {
                        isImage && (
                            <a
                                href={fileUrl}
                    ...
                    {/* if we have a file url and the type is pdf */}
                    {
                        isPDF && <div
                    ...
                    {/* if the message is not of typpe fileurl and we are not currently editing the message
                    ...
                    */}
                    ...
                    {/* form area to edit the current message */}
                    {!fileUrl && isEditing && (
                        <Form {...form}>
                            <form
            ...
            {/* if the message is not deleted then we can delete the message */}
            {
                canDeleteMessage && (
                    <div
                            ...
                            canEditMessage && (
                                ...
                                >
                                    <Edit
                                        ...
                                        className="cursor-pointer ml-auto w-4 h-4 text-zinc-600 dark:hover:text-zinc-300 transition"
                                    />
                                </ActionTooltip>
                            )
                        }
